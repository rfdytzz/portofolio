    ...
    <div>
        <div class="relative" ref="containerRef">
            <nav class="flex relative" :style="{ transform: 'translate3d(0,0,0.01px)' }">
                <ul ref="navRef" class="flex gap-8 list-none p-0 px-4 m-0 relative z-3" :style="{
                    color: 'white',
                    ...
                    <li v-for="(item, index) in items" :key="index" :class="[
                        ...
                        <a :href="item.href || undefined" @click="e => handleClick(e, index)"
...
@keyframes particle {
    0% {
        transform: rotate(0deg) translate(var(--start-x),
                var(--start-y));
        opacity: 1;
    ...
    70% {
        transform: rotate(calc(var(--rotate) * 0.5)) translate(calc(var(--end-x) * 1.2),
                calc(var(--end-y) * 1.2));
        opacity: 1;
    ...
    85% {
        transform: rotate(calc(var(--rotate) * 0.66)) translate(var(--end-x),
                var(--end-y));
        opacity: 1;
    }

    100% {
        transform: rotate(calc(var(--rotate) * 1.2)) translate(calc(var(--end-x) * 0.5),
                calc(var(--end-y) * 0.5));
        opacity: 1;
    }
}
