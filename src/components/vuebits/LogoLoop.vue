<template>
    <div ref="containerRef" :class="rootClasses" :style="containerStyle" role="region" :aria-label="ariaLabel">
        ...
        <div ref="trackRef" :class="[
            'flex will-change-transform select-none relative z-0',
            'motion-reduce:transform-none',
            isVertical ? 'flex-col h-max w-full' : 'flex-row w-max'
        ]" @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave">
            <ul v-for="copyIndex in copyCount" :key="`copy-${copyIndex - 1}`"
                :class="['flex items-center', isVertical && 'flex-col']" role="list"
                :aria-hidden="copyIndex > 1 ? true : undefined" :ref="el => {
                ...
                <li v-for="(item, itemIndex) in logos" :key="`${copyIndex - 1}-${itemIndex}`" :class="[
                    ...
                    <slot v-if="$slots.renderItem" name="renderItem" :item="item"
